...
Piece.prototype.new = function(index){
    ...
    if(!this.moveValid(0, 0, this.tetro)){
        gameState = 9;
        $setText(msg,lang("BLOCK OUT!","遊戲結束"));
        menu(3);
        return;
    }
...
Piece.prototype.checkShift = function(){
    ...
    if(this.shiftDir){
        ...
        // 2. Apply DAS delay
            ...
            this.shiftDelay++;
        // 3. Once the delay is complete, move over once. Increment delay so this doesn't run again.
        }else if(this.shiftDelay === settings.DAS){
            this.shift(this.shiftDir);
...
Piece.prototype.drawGhost = function(){
    ...
    if(settings.Ghost === 0 && !landed){
        draw(this.tetro, this.x, Math.floor(this.y + this.getDrop(2147483647)) - stack.hiddenHeight, activeCtx, 0);
    }else if(settings.Ghost === 1 && !landed){
        draw(this.tetro, this.x, Math.floor(this.y + this.getDrop(2147483647)) - stack.hiddenHeight, activeCtx, RotSys[settings.RotSys].color[this.index]);
    }
